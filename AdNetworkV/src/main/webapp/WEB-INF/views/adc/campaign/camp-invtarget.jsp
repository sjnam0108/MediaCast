...
<c:url value="/adc/campaign/invtargets/read" var="readUrl" />
<c:url value="/adc/campaign/invtargets/readOrdered" var="readOrderedUrl" />
<c:url value="/adc/campaign/invtargets/reorder" var="reorderUrl" />
<c:url value="/adc/campaign/invtargets/calcScrCount" var="calcScrCountUrl" />
<c:url value="/adc/campaign/invtargets/updateFilterType" var="updateFilterTypeUrl" />
<c:url value="/adc/campaign/invtargets/destroy" var="destroyUrl" />

<c:url value="/adc/campaign/invtargets/readACRegion" var="readRegionUrl" />
<c:url value="/adc/campaign/invtargets/readACScreen" var="readScreenUrl" />
<c:url value="/adc/campaign/invtargets/readACState" var="readStateUrl" />
<c:url value="/adc/campaign/invtargets/readACSite" var="readSiteUrl" />
<c:url value="/adc/campaign/invtargets/readACSiteCond" var="readSiteCondUrl" />

<c:url value="/adc/campaign/invtargets/saveRegion" var="saveRegionUrl" />
<c:url value="/adc/campaign/invtargets/saveScreen" var="saveScreenUrl" />
<c:url value="/adc/campaign/invtargets/saveState" var="saveStateUrl" />
<c:url value="/adc/campaign/invtargets/saveSite" var="saveSiteUrl" />
<c:url value="/adc/campaign/invtargets/saveSiteCond" var="saveSiteCondUrl" />
...
<h4 class="pt-1 pb-3 mb-3">
	<span class="mr-1 fa-light fa-bullseye-arrow fa-fw"></span>
	${pageTitle}
	<span class="font-weight-light pl-1">인벤토리 타겟팅</span>
</h4>
...
<ul class="nav nav-tabs tabs-alt mb-4 mt-3">
	...
	<li class="nav-item">
		<a class="nav-link active" href="/adc/campaign/invtargets/${Campaign.id}">
...
$(document).ready(function() {
	...
	$("select[name='nav-item-ad-select']").on("change.bs.select", function(e){
		location.href = "/adc/campaign/invtargets/${Campaign.id}/" + $("select[name='nav-item-ad-select']").val();
	});
...
<%
	...
	String tgtDispTemplate =
			...
			"<button type='button' onclick='editInvTgt(#= id #)' class='btn icon-btn btn-sm btn-outline-success borderless'>" + 
			"<span class='fas fa-pencil-alt'></span></button>";
			
	String geoLocTemplate =
			"<a href='javascript:showGeoLoc(\"ADINVEN\", #= id #)' class='btn btn-default btn-xs icon-btn'>" +
...
<div class="mb-4">
<kendo:grid name="grid" pageable="true" filterable="false" sortable="false" scrollable="true" reorderable="true" resizable="true" selectable="${value_gridSelectable}" >
...
function calcTargetScreenCnt() {
	...
	$("#inven-target-tab-title").text("인벤토리 타겟팅(계산중)");
	...
	$.ajax({
		...
		success: function (data) {
			...
			$("#inven-target-tab-title").text("인벤토리 타겟팅(" + data + ")");
		},
		error: ajaxOperationError
	});
}
...
function editInvTgt(id) {
	...
	if (dataItem.invenType == "RG") {
		...
		validateSiteValue($("#form-15 select[name='siteConds']").data("kendoMultiSelect").value());
